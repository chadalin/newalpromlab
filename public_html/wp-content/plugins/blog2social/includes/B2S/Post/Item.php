    ...
    protected function getData() {
        ...
        if (!empty($this->searchPostShareStatus)) {
            ...
            if ($this->searchPostShareStatus == 'shared') {
                ...
                //never
                $leftJoinWhere .= " AND (b2s.post_id IS NULL OR b2s.hide = 1)";
            }
        }
                                ...
                                <div class="media">
                                    <img class="post-img-10 pull-left hidden-xs" src="' . plugins_url('/assets/images/b2s/' . $postType . '-icon.png', B2S_PLUGIN_FILE) . '" alt="posttype">
                                        <div class="media-body">
                                            ...
                                            <p class="info hidden-xs">#' . esc_html($var->ID . ' | ' . __('Author', 'blog2social')) . ' <a href="' . esc_url(get_author_posts_url($var->post_author)) . '">' . esc_html((isset($userInfo['nickname'][0]) ? $userInfo['nickname'][0] : '-')) . '</a> | ' . esc_html__('saved', 'blog2social') . ': ' . esc_html(B2S_Util::getCustomDateFormat($var->post_date, substr(B2S_LANGUAGE, 0, 2)) . $lastPublish) . '</p>
                                        </div>
                                    </div>
                                </li>';
            }
                ...
                $this->postItem .= '<li class="list-group-item b2s-draft-list-entry" data-b2s-draft-id="' . esc_attr($var->draft_id) . '">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        ...
                                        <p class="info hidden-xs">#' . esc_html($var->ID . ' | ' . __('Author', 'blog2social')) . ' <a href="' . esc_url(get_author_posts_url($var->post_author)) . '">' . esc_html((isset($userInfo['nickname'][0]) ? $userInfo['nickname'][0] : '-')) . '</a> | ' . esc_html__('last saved', 'blog2social') . ': ' . esc_html(B2S_Util::getCustomDateFormat($var->last_save_date, substr(B2S_LANGUAGE, 0, 2))) . '</p>
                                    </div>
                                </div>
                            </li>';
            }
